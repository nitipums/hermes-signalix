...
- `/mvp` cards show `Primary Daily Wave` from `wave.primary_state` and the existing decision lane. The additive `wave.daily_structure` evidence remains in the API/audit contract but is temporarily hidden from the main review surface because it confuses the primary-wave hierarchy.
- The drawer keeps the primary wave, confidence, setup, and evidence/provenance surfaces without a Daily structural context field. Daily structural evidence is available only through backend/read-model/audit paths until the owner approves a new presentation.
- The main `/mvp` list is grouped by existing decision lane only; it does not group or filter by `wave.daily_structure.phase`. This is presentation-only and must not change the server-side evaluated universe, lane totals, pagination, `wave.primary_state`, or decision lanes.
- `wave.context.mapped_state` remains compatibility/audit evidence only and is not used for main grouping or filtering. `Primary Daily Structure` and `Daily structure · <phase>` are not user-facing labels in the current `/mvp` surface.
