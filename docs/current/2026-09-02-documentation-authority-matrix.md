...
| Architecture/data flow | `vault/Architecture.md` | `vault/Components.md`, source tree | old flow sections/postmortems | Retired dashboard/snapshot flow is labelled compatibility/history; canonical dispatcher, projection, chart-read, and freshness-lineage seams are explicit |
...
- Runtime refactor is complete for the route dispatcher, canonical projection, chart-read, and freshness-lineage seams; further legacy deletion remains a separate bounded decision.
