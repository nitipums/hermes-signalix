...
**Status:** DONE — source/test/runtime/API/browser/mobile PASS; public external ingress NOT VERIFIED (no configured public URL)
...
- [x] Happy path, drawer/chart path, stale/blocked/unknown path, and browser console/page-error checks pass on the actual served route. Local canonical `/wave-context` happy path, chart selection, and `/mvp` preservation passed; public external ingress is not configured.
