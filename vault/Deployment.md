...
- `signalix-intraday.timer` — every 30 minutes in guarded Bangkok weekday time blocks; `signalix-intraday.service` fetches active 60m and, after committed full/partial ingestion, runs isolated VCP Finder with ingestion lineage before the existing evaluator. Daily Shortlist is not part of the primary MVP surface. The VCP run is never triggered from `ExecStopPost`; failed/skipped fetches do not advance VCP. Validate `OnCalendar` with `systemd-analyze calendar` after edits.
