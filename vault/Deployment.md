...
- **Wave 1 Market Breadth companion promotion — 2026-09-19:** Source commits `898489b`, `e0ac407`, `75092d5`, and `9876724` were fast-forwarded onto `release/signalix-mvp-stable` from `/root/signalix`. The companion read-only page is `/market-breadth` with `/api/market-breadth`; top navigation links it with `/trend-map` in both directions. The generated artifact was rebuilt from the production PostgreSQL source with `PYTHONPATH=backend python -m market_breadth_publisher --read-only --root backend/market-breadth-read-model` and validated as `signalix.market-breadth.artifact.v2`, hash `8613c4d194758f5baa3384ea8c84f5f1088b1f9e97ec83ac5c70dd797c7058c4`, `as_of=2026-09-18`, `929` active symbols, `841` observed, `88` blocked, `quality=PARTIAL`, `freshness=AVAILABLE`, `260` report sessions. `docker compose up -d --force-recreate dashboard` was run from `/root/signalix`; Compose recreated `signalix_backend` and `signalix_dashboard`, while PostgreSQL and Redis remained running. Readiness returned `{"status":"ok","db":"up","redis":"up"}`. Local and public Market Breadth API returned HTTP 200, `PRODUCTION_READ_ONLY`, `PARTIAL`, and the same coverage; public 390px browser read-back verified both navigation links, active state, no overflow, as-of/quality/coverage, and `range=20` loading. Residual: public `/api/trend-map` remains HTTP 200 but `DATA_BLOCKED`, `verification_status=NOT_VERIFIED`, freshness `STALE`, as_of `2026-09-15`; this pre-existing Trend Map artifact freshness issue remains open and is not attributed to the Market Breadth promotion. Git source is local ahead by two commits; generated Market Breadth artifacts remain untracked runtime inputs; no push performed.
