...
- Owner decision (2026-09-12): the Daily trend-map shadow surface is permanently public read-only. `GET /trend-map-shadow` and same-origin `GET /api/trend-map-shadow` require no authentication. This surface is research evidence only: it has no setup, recommendation, alert, order, broker, BUY, or other action semantics, and this decision does not change `/mvp`'s separate owner-only/private signal policy.
- Current hardening source and deployment read-back (2026-09-12): bounded publisher, artifact/failure-sidecar validation, EOD hook, and fail-closed SQL guard are deployed. Public shadow API returned HTTP 200 with 237 rows, `229 AVAILABLE / 5 INVALID_DATA / 3 NO_DATA`, `FRESH`, artifact read latency `0.232s`, and persisted timing/hash metadata. Full browser error/recovery remains `NOT VERIFIED`.
...
- Status: source implementation, focused tests, deployed/public artifact read-back, and `/mvp` preservation are `PASS`. The Daily trend-map shadow route is permanently public read-only with no authentication required; full browser error/recovery remains `NOT VERIFIED`. Shadow remains non-actionable research evidence; no production decision/action path changed.
- Worktree note: untracked `research/`, `docs/current/` QA/session notes, and generated `backend/shadow-read-model/` artifacts are preserved owner/task artifacts for research, QA, replay, or read-back evidence. They are not additional production code changes in this bounded hardening diff and must not be cleaned, reset, or treated as deployment evidence.
