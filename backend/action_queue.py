...
def assign_action_queue(stage, phase, quality_pass, proximity_state,
    ...
    # R3 qualified-retest hard gate (t_3ae98ae4): evaluated FIRST so a
    # confirmed breakout-retest event lands in retest_watch, not the
    # emerging-event lane. Requires persisted event/provenance evidence
    # (an active event carrying its original trigger price); the phase
    # label alone never qualifies.
    if phase == "breakout_retest":
        if isinstance(intraday_event, dict) \
                and intraday_event.get("trigger_price") \
                and intraday_event.get("status") in (None, "active"):
            return "retest_watch"
        return "monitor_only"
    ...
    if stage == "S2_uptrend":
        ...
        if phase == "breakout_retest":
            return "monitor_only" if not intraday_event else "retest_watch"  # unreachable: gated above
