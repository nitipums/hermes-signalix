"""
Signalix Phase 2 — Minervini Trend Template Scanner (Thai stocks)
Pure deterministic computation with pandas. NO LLM here.
Outputs candidate signals that Phase 3 (LLM) will summarize and Phase 4 (bots) will push.

Minervini Trend Template (8 conditions):
 1. Price >= 150-day MA
 2. 150-day MA >= 200-day MA
 3. 200-day MA trending up (>= 1mo ago)
 4. 50-day MA >= 150-day MA
 5. Price >= 50-day MA
 6. Price >= prior 52-week low * 1.25  (at least 25% above 52w low)
 7. Price within 25% of 52-week high    (near, not far below, the high)
 8. RS Rating >= 70                      (relative strength vs market)
Plus: VCP detection (volatility contraction) and Position Sizing.
"""
import math
...
def analyze_symbol(symbol: str, df: pd.DataFrame, market_series: pd.Series) -> dict:
    """Full pipeline for one symbol. market_series = SET close Series."""
    close = _close_series(df)
    rs = compute_rs_rating(close, market_series)
    tt = trend_template(df, rs)
    vcp = detect_vcp(df["High"], df["Low"], period=VCP_PERIOD)
    bz = buy_zone(df)
    recent_low = df["Low"].tail(10).min()
    suggested_stop = min(recent_low, df["Close"].iloc[-1] * 0.94)
...
if __name__ == "__main__":
    ...
    universe = ["PTT.BK", "KBANK.BK", "DELTA.BK", "CPALL.BK", "SCB.BK", "ADVANC.BK", "BDMS.BK", "AOT.BK"]
    market = yf.download("^SET.BK", period="2y", interval="1d", progress=False, auto_adjust=True)["Close"]
    print(f"Market ^SET.BK rows={len(market)}")
    results = []
    for s in universe:
        df = yf.download(s, period="2y", interval="1d", progress=False, auto_adjust=True)
        if df is None or len(df) < MIN_DAYS:
            continue
        r = analyze_symbol(s, df, market)
        results.append(r)
        tt = r["trend_template"]
        print(f"{s:10s} RS={tt['rs_rating']:5.1f}  TT={tt['conditions_met']}/8  "
              f"VCP={r['vcp']['is_vcp']}  PASS={tt['pass']}  stop={r['suggested_stop']}")
    passed = [r for r in results if r["trend_template"]["pass"]]
    print(f"\nCandidates passing full Trend Template: {len(passed)}/{len(results)}")
